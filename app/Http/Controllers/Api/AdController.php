...
class AdController extends ApiResponse
{
    ...
    {
        $ad_details = $ad->load('category', 'subcategory', 'customer', 'brand', 'adFeatures', 'galleries', 'city', 'town')->loadCount(['wishlists as wishlisted' => function ($q) {
            $q->where('user_id',  auth('api')->id());
        }]);
        $related_ads = Ad::with('city')->whereCategoryId($ad->category_id)->where('id', '!=', $ad->id)->latest()->limit(4)->get();
