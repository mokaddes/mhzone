                            ...
                            <div class="col-sm-12 col-md-3">
                                <select name="filter_by" class="form-control w-100-p">
                                    ...
                                    @if ($settings->ads_admin_approval)
                                        <option
                                            {{ request('filter_by') == 'pending' ? 'selected' : '' }} value="pending">
                                            {{ __('pending') }}</option>
                                    @endif
...
@section('script')
    ...
    <script>
        function calculateFinalPrice(id) {
            let originalPrice = parseFloat($('#price' + id).val());
            let discount = parseFloat($('#discount' + id).val());
            if (discount > 99) {
                $('#discount' + id).val(99);
                discount = 99 ;
            }
            ...
            $('#price_after_discount' + id).val(finalPrice.toFixed(2));
        }
    </script>
@endsection
