                        ...
                        <div class="col-md-6 offset-md-3">
                            <form class="form-horizontal" action="{{ route('module.category.update', $category->id) }}"
                                ...
                                <div class="form-group row">
                                    <x-forms.label name="Department" required="true" class="col-sm-3 col-form-label" />
                                    <div class="col-sm-9">
                                        <select name="department_id" id="department_id" class="form-control @error('department_id') is-invalid @enderror">
                                            @if(isset($departments) && $departments->count() > 0)
                                            <option value="" hidden>Select a Department</option>
                                                @foreach($departments as $value)
                                                    <option value="{{ $value->id }}" {{ $category->department_id == $value->id? "selected" : "" }}>{{ $value->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                        @error('department_id')
